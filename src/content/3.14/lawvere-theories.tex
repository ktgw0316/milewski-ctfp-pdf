...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[->,auto,bend angle=30,label position=right]
    \node (F) at (0,3.7) {$\mathbf{F}$};
    \node (Fop) at (2,3.7) {$\mathbf{F}^{op}$};
    \node (L) at (4,3.7) {$\mathbf{L}$};
    \node (Set) at (6,3.7) {$\mathbf{Set}$};
    \draw (Fop) to node {$\mathbf{I}_L$} (L);
    \draw (L) to node {$\mathbf{M}$} (Set);
    \foreach \i in{0,...,3} {
      \node[draw,rounded corners=6pt,outer sep=2pt] (\i) at (0,\i) {\foreach \i in{0,...,\i} {$\bullet$}};
      \node[anchor=west] at (0.5,\i) {\i};
    }
    \draw (0.west) to[bend left] (3.west);
    \draw (0.north west) to[bend left] (2.west);
    ...
    \foreach \j in{2,4} {
      \foreach \i in{0,...,3} {
        \node[label=\i] (\i) at (\j,\i) {$\bullet$};
      }
      \draw (3.west) to[bend right] (0.west);
      \draw (2.west) to[bend right] (0.north west);
      \draw (2) to[bend right] (1);
      \draw (2) to[bend left] (1);
      \draw (1) to (2);
      \draw (1) to (0);
    }
    \draw[dashed][transform canvas={xshift=2pt}] (2.south east) to[bend left] (1.north east);
    \draw[dashed][transform canvas={xshift=6pt}] (2.south east) to[bend left] (1.north east);
    \draw[dashed][transform canvas={xshift=10pt}] (2.south east) to[bend left] (1.north east);
    \draw[dashed] (0) to[bend right] (1.south east);
    \node[label={$\mathbf{M3} = (\mathbf{M1})^3$}] (3) at (6,3) {$\bullet$};
    \node[label={$\mathbf{M2} = (\mathbf{M1})^2$}] (2) at (6,2) {$\bullet$};
    \node[label={$\mathbf{M1}$}] (1) at (6,1) {$\bullet$};
    \node[label={$\mathbf{M0}$}] (0) at (6,0) {$\bullet$};
    \draw (3.west) to[bend right] (0.west);
    \draw (2) to[bend right] (1);
    \draw (2) to[bend left] (1);
    \draw (1) to (2);
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[->,auto,bend angle=30,label position=right]
    \draw (2,4) circle[x radius=0.5,y radius=0.7];
    \node at (1.5,4.7) {$a^m$};
    \draw (4,4) circle[x radius=0.6,y radius=0.8];
    \node at (4.7,4.7) {$a^n$};
    \node[draw,rounded corners=6pt,rotate=90] (am) at (2,4) {$\bullet \bullet$};
    \node[draw,rounded corners=6pt,rotate=90] (an) at (4,4) {$\bullet \bullet \bullet \bullet$};
    \draw (an.north east) to (am);
    \draw (an) to (am.south west);
    \node[draw,rounded corners=6pt,outer sep=2pt,rotate=90,label=m] (m) at (2,2) {$\bullet \bullet$};
    \node[draw,rounded corners=6pt,outer sep=2pt,rotate=90,label=n] (n) at (4,2) {$\bullet \bullet \bullet \bullet$};
    \draw (m) to (n.north east);
